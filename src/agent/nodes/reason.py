...
def _select_answer_contexts(contexts: list[dict[str, Any]], limit: int = 5) -> list[dict[str, Any]]:
    main_contexts = [ctx for ctx in contexts if not _is_reference_context(ctx)]
    reference_contexts = [ctx for ctx in contexts if _is_reference_context(ctx)]
    selected = main_contexts[:limit]
    if len(selected) < limit:
        selected.extend(reference_contexts[: limit - len(selected)])
    return selected
...
async def generate_answer_node(state: ClinicalState) -> dict:
    ...
    try:
        ...
        answer_contexts = _select_answer_contexts(contexts, limit=3)
